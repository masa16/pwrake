...
module Pwrake
  ...
  # Filled Highest Rank First

  class FhrfQueueArray < Array
    ...
    def shift(host_info, min_core)
      ...
      size.times do |i|
        ...
        if tw.tried_host?(host_info)
          ...
          r = tw.rank
          i_rank[r] ||= i
          c = (count[r] || 0) + tw.use_cores(host_info)
          if c >= @nproc
            Log.debug "qa7: task=#{tw.name} count_core=#{c} i=#{i}/#{size} rank=#{r}"
            return q_delete_at(i_rank[r])
          end
          count[r] = c
        end
      end
      unless i_rank.empty?
        tw = q_delete_at(i=i_rank.last)
        Log.debug "qa9: task=#{tw.name} count_core=#{count.last} i=#{i}/#{size} rank=#{i_rank.size-1}"
        return tw
      end
      i_tried.each do |i|
        tw = q_at(i)
        if tw.acceptable_for(host_info)
          use_core = tw.use_cores(host_info)
          Log.debug "qa8: task=#{tw.name} use_core=#{use_core} i=#{i}/#{size}"
          return q_delete_at(i)
        end
      end
      nil
  ...
  class FhrfLifoQueueArray < FhrfQueueArray
    def q_at(i)
      at(size-1-i)
    end
    def q_delete_at(i)
